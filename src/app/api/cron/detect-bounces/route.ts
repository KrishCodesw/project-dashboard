...
import { detectBounces } from "@/lib/delivery/detectBounces";
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await detectBounces();
    return NextResponse.json({ ok: true, ...result });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Bounce detection failed";
